...
// Optional fields (alleen als je ze in index.html hebt)
const placeInput = $("place");      // kan null zijn
const chartImgInput = $("chartImg"); // kan null zijn
...
// ================= Kleine helpers (anti-dubbel / veilig) =================
...
function houseFocusText(houseNr, sign){
  ...
  const cleanKey = stripPrefix(s.key, "Sleutel:");
  return `${HOUSE_MEANING[houseNr]} In ${sign}: je benadert dit met ${s.core}. Sleutel: ${cleanKey}`;
}
...
// let op: géén numerologie meer in deze zin (anders dubbel)
...
function parseInputs(){
  const name = safeText($("name")?.value, "").trim();
  const place = placeInput ? safeText(placeInput.value, "").trim() : "";
  const dateStr = safeText($("date")?.value, "");
  const timeStr = safeText($("time")?.value, "");
  const tz = Number($("tz")?.value);
  const lat = Number($("lat")?.value);
  const lon = Number($("lon")?.value);
  const hsys = safeText($("hsys")?.value, "P");
  ...
  if(Number.isNaN(pages)) throw new Error("Pagina-keuze is niet geldig.");
...
async function init(){
  ...
  // defaults (alleen als leeg)
  if(!$("date")?.value){
    if($("name")) $("name").value = "marit visser";
    if(placeInput) placeInput.value = "Harmelen, Utrecht, Nederland";
...
function generatePdf(){
  ...
  // chart image (if uploaded) — optioneel
  ...
  const rawPit = SIGN_TEXT[moonSign]?.shadow || "een terugkerend patroon om bewust te dragen";
  const pitfall  = `Valkuil: ${rawPit}`;
  ...
  if(totalPages >= 2){
    ...
    for(const pName of corePlanets){
      ...
      const shadowClean = stripPrefix(st?.key, "Sleutel:");
      const keyLine = shadowClean ? `Sleutel: ${shadowClean}` : "";
  ...
  if(totalPages >= 4){
    ...
    const vKey = `Sleutel: ${stripPrefix(SIGN_TEXT[venusSign]?.key || "", "Sleutel:")}`;
    const mKey = `Sleutel: ${stripPrefix(SIGN_TEXT[marsSign]?.key || "", "Sleutel:")}`;
  ...
  if(totalPages >= 5){
    ...
    y = addWrappedText(doc,
      ...
      `Sleutel: ${stripPrefix(SIGN_TEXT[jSign]?.key || "Sleutel: werk met vertrouwen.", "Sleutel:")}`,
      marginX, y, maxW, 5
    );
    ...
    y = addWrappedText(doc,
      ...
      `Sleutel: ${stripPrefix(SIGN_TEXT[sSign]?.key || "Sleutel: bouw in kleine stappen.", "Sleutel:")}`,
      marginX, y, maxW, 5
    );
    ...
    y = addWrappedText(doc,
      `Wanneer je groei (${jSign}) koppelt aan structuur (${sSign}), ontstaat duurzame kracht. Kies 1 vaardigheid om te trainen en 1 routine om 7 dagen vol te houden.`,
      marginX, y, maxW, 5
    );
