...
// ================= Kleine helpers (anti-dubbel) =================
function stripPrefix(text, prefix){
  if(!text) return "";
  const t = String(text).trim();
  const p = String(prefix).trim();
  return t.toLowerCase().startsWith(p.toLowerCase()) ? t.slice(p.length).trim() : t;
}
...
// ================= Text modules (Nieuwe Alchemist - compact) =================
...
// 2) Teken = stijl (incl. schaduw + sleutel)
const SIGN_TEXT = {
  Ram: {
...
// 2b) Jupiter & Saturnus (kort)
const JUPITER_TEXT = {
  Ram: "Groei komt via initiatief, lef en kiezen voor jezelf.",
  Stier: "Groei komt via stabiliteit, geduld en duurzaam opbouwen.",
  Tweelingen: "Groei komt via leren, verbinden en je stem gebruiken.",
  Kreeft: "Groei komt via veiligheid, intuïtie en emotionele bedding.",
  Leeuw: "Groei komt via creatie, zichtbaarheid en hart-leiderschap.",
  Maagd: "Groei komt via vakmanschap, verfijning en praktische verbetering.",
  Weegschaal: "Groei komt via samenwerking, balans en relationele wijsheid.",
  Schorpioen: "Groei komt via diepgang, eerlijkheid en transformatiekracht.",
  Boogschutter: "Groei komt via visie, zingeving en vertrouwen in het grotere plaatje.",
  Steenbok: "Groei komt via verantwoordelijkheid, structuur en lange termijn.",
  Waterman: "Groei komt via originaliteit, vernieuwing en community.",
  Vissen: "Groei komt via empathie, verbeelding en innerlijk weten."
};

const SATURN_TEXT = {
  Ram: "De les is tempo beheersen: kracht met richting, niet alleen snelheid.",
  Stier: "De les is loslaten van controle: flexibiliteit ontwikkelen zonder onrust.",
  Tweelingen: "De les is focus: minder versnippering, meer verdieping en afronden.",
  Kreeft: "De les is grenzen: niet alles dragen, bewust kiezen wat van jou is.",
  Leeuw: "De les is eigenwaarde: stralen zonder afhankelijk te zijn van bevestiging.",
  Maagd: "De les is mildheid: perfectionisme ombuigen naar gezonde standaarden.",
  Weegschaal: "De les is kiezen: harmonie zonder jezelf te verliezen.",
  Schorpioen: "De les is vertrouwen: controle transformeren naar openheid met grenzen.",
  Boogschutter: "De les is realiteitszin: visie koppelen aan discipline en stappen.",
  Steenbok: "De les is zachtheid: verantwoordelijkheid dragen zonder verkramping.",
  Waterman: "De les is verbinden: onafhankelijk zonder emotioneel afstandelijk te worden.",
  Vissen: "De les is gronding: gevoeligheid kanaliseren zonder escapisme."
};
...
function houseFocusText(houseNr, sign){
  ...
  if(!s) return `${HOUSE_MEANING[houseNr]} Dit gebied vraagt om bewuste aandacht en ontwikkeling.`;
  const k = stripPrefix(s.key, "Sleutel:");
  return `${HOUSE_MEANING[houseNr]} In ${sign}: je benadert dit met ${s.core}. Sleutel: ${k}`;
}

// 4) Reflectievragen (kort)
function questionsForRelationship(){
  return [
    "Wat heb ik nodig om veilig én vrij te blijven in verbinding?",
    "Waar pas ik me aan uit angst voor conflict, en wat is mijn waarheid?"
  ];
}
function questionsForDrive(){
  return [
    "Wat is mijn natuurlijke tempo als ik niet ‘moet’ van mezelf?",
    "Wat vermijd ik door ‘aardig’ te blijven — en wat wil ik echt?"
  ];
}
function questionsForTalent(){
  return [
    "Waar gaat groei moeiteloos als ik consistent blijf?",
    "Welke structuur maakt mijn potentieel concreet?"
  ];
}

// ================= Tarot archetypes (Life Path 1–22) =================
const LIFE_PATH_ARCHETYPE = {
  1: { card:"De Magiër", text:"Kern: je bent hier om richting te geven en iets in beweging te zetten. Dynamiek: je leert door initiatief en zelfvertrouwen. Valkuil: twijfelen aan je eigen kracht of manipuleren. Sleutel: focus en handel vanuit helder intentie." },
  2: { card:"De Hogepriesteres", text:"Kern: jouw kracht ligt in waarnemen en innerlijk weten. Dynamiek: je voelt voordat je spreekt. Valkuil: jezelf onzichtbaar maken of blijven wachten. Sleutel: vertrouw je intuïtie en geef er woorden aan." },
  3: { card:"De Keizerin", text:"Kern: je brengt groei, creatie en verbinding. Dynamiek: je voedt mensen en ideeën. Valkuil: pleasen of jezelf verliezen in zorg. Sleutel: creëer vanuit overvloed, niet uit tekort." },
  4: { card:"De Keizer", text:"Kern: je bouwt structuur en stabiliteit. Dynamiek: je neemt verantwoordelijkheid en zet kaders. Valkuil: controle of verharding. Sleutel: leid met helderheid én menselijkheid." },
  5: { card:"De Hierofant", text:"Kern: je draagt kennis en betekenis over. Dynamiek: je verbindt ervaring met wijsheid. Valkuil: dogma of gelijk willen hebben. Sleutel: blijf leerling terwijl je leraar bent." },
  6: { card:"De Geliefden", text:"Kern: jouw pad draait om verbinding en keuze. Dynamiek: relaties spiegelen je innerlijke waarheid. Valkuil: aanpassen of besluiteloosheid. Sleutel: kies vanuit authenticiteit." },
  7: { card:"De Zegewagen", text:"Kern: je ontwikkelt meesterschap door richting en discipline. Dynamiek: je leert door beweging en uitdaging. Valkuil: forceren of jezelf voorbijlopen. Sleutel: stuur met bewustzijn, niet met strijd." },
  8: { card:"Kracht", text:"Kern: je bezit innerlijke kracht en zachte beheersing. Dynamiek: je beïnvloedt zonder te domineren. Valkuil: onderdrukte woede of machtsspel. Sleutel: blijf aanwezig in spanning." },
  9: { card:"De Kluizenaar", text:"Kern: je zoekt waarheid van binnenuit. Dynamiek: je verdiept door stilte en reflectie. Valkuil: isolatie of afstand. Sleutel: deel je inzichten wanneer ze rijp zijn." },
  10:{ card:"Rad van Fortuin", text:"Kern: jouw leven beweegt in cycli van verandering. Dynamiek: je leert meebewegen met timing. Valkuil: controle willen houden. Sleutel: vertrouw het proces en handel bewust." },
  11:{ card:"Gerechtigheid", text:"Kern: je bent gericht op waarheid en evenwicht. Dynamiek: je ziet verbanden en weegt zorgvuldig af. Valkuil: strengheid of oordeel. Sleutel: waarheid met compassie." },
  12:{ card:"De Gehangene", text:"Kern: je leert door loslaten en perspectiefwisseling. Dynamiek: stilstand brengt inzicht. Valkuil: slachtofferhouding of uitstel. Sleutel: kies bewust voor overgave." },
  13:{ card:"Dood", text:"Kern: jouw pad vraagt om diepe transformatie. Dynamiek: eindes maken ruimte voor vernieuwing. Valkuil: vasthouden aan het oude. Sleutel: durf af te sluiten." },
  14:{ card:"Gematigdheid", text:"Kern: je brengt balans en integratie. Dynamiek: je verbindt uitersten tot een geheel. Valkuil: jezelf wegcijferen. Sleutel: blijf afgestemd op jouw midden." },
  15:{ card:"De Duivel", text:"Kern: je leert omgaan met verlangen en macht. Dynamiek: schaduwen vragen bewustzijn. Valkuil: verslaving of afhankelijkheid. Sleutel: kies vrijheid via verantwoordelijkheid." },
  16:{ card:"De Toren", text:"Kern: jouw groei komt via doorbraken. Dynamiek: oude structuren vallen weg. Valkuil: vasthouden aan schijnveiligheid. Sleutel: bouw opnieuw vanuit waarheid." },
  17:{ card:"De Ster", text:"Kern: je brengt hoop en herstel. Dynamiek: jouw aanwezigheid werkt helend. Valkuil: jezelf vergeten. Sleutel: blijf verbonden met je eigen bron." },
  18:{ card:"De Maan", text:"Kern: je beweegt door het onderbewuste. Dynamiek: intuïtie en gevoel sturen je pad. Valkuil: verdwalen in angst of verwarring. Sleutel: vertrouw wat je voelt, maar toets het aan realiteit." },
  19:{ card:"De Zon", text:"Kern: je bent hier om te stralen en leven te vieren. Dynamiek: zichtbaarheid geeft energie. Valkuil: afhankelijkheid van erkenning. Sleutel: straal van binnenuit." },
  20:{ card:"Oordeel", text:"Kern: je wordt geroepen tot ontwaken. Dynamiek: oude thema’s vragen afronding. Valkuil: blijven hangen in schuld of verleden. Sleutel: antwoord op je roep." },
  21:{ card:"De Wereld", text:"Kern: je belichaamt voltooiing en integratie. Dynamiek: alles valt samen. Valkuil: leegte na afronding. Sleutel: erken wat je hebt gerealiseerd." },
  22:{ card:"De Dwaas", text:"Kern: jouw pad vraagt vertrouwen in het onbekende. Dynamiek: je leert door ervaring en sprongen. Valkuil: roekeloosheid of verantwoordelijkheid vermijden. Sleutel: durf te beginnen, met bewustzijn." }
};

function normalizeLifePathForTarot(lp){
  // we houden 11/22 als master; 33 bestaat in numerologie maar tarot-archetypen zijn 1–22
  if(lp === 33) return 6; // 3+3=6 (Geliefden) als archetypische basis
  return lp;
}

function getLifePathTarot(lp){
  const key = normalizeLifePathForTarot(lp);
  return LIFE_PATH_ARCHETYPE[key] || null;
}
...
function makeCoreSentence(sunSign, ascSign){
  return `Zon in ${sunSign} als kern, Ascendant in ${ascSign} als uitstraling.`;
}

// ================= Inputs =================
function parseInputs(){
  const name = safe($("name")?.value).trim();
  const place = safe($("place")?.value).trim();
  const dateStr = safe($("date")?.value);
  const timeStr = safe($("time")?.value);
  const tz = Number($("tz")?.value);
  const lat = Number($("lat")?.value);
  const lon = Number($("lon")?.value);
  const hsys = safe($("hsys")?.value) || "P";
  const pages = Number($("pages")?.value || 5);
...
async function init(){
  ...
  // defaults (handig)
  if(!$("date")?.value){
    if($("name")) $("name").value = "marit visser";
    if($("place")) $("place").value = "Harmelen, Utrecht, Nederland";
    if($("date")) $("date").value = "1980-08-04";
    if($("time")) $("time").value = "02:25";
    if($("lat"))  $("lat").value  = "52.0882";
    if($("lon"))  $("lon").value  = "4.9633";
    if($("tz"))   $("tz").value   = "2";
  }
...
function renderHouses(cusps, ascLon, mcLon){
  ...
  if(ascEl) ascEl.textContent = `${ascS.sign} ${fmtDeg(ascS.within)} (${fmtDeg(ascLon)})`;
  if(mcEl)  mcEl.textContent  = `${mcS.sign} ${fmtDeg(mcS.within)} (${fmtDeg(mcLon)})`;
}

function renderNumerology(n){
  if(lpEl) lpEl.textContent = n.lifePath;
  if(ndayEl) ndayEl.textContent = n.day;
  if(nmonEl) nmonEl.textContent = n.month;
  if(nyearEl) nyearEl.textContent = n.year;
}
...
function generatePdf(){
  ...
  if(chartImageDataUrl && chartImageFormat){
    ...
    try{
      ...
      y = addWrappedText(doc, "⚠️ Horoscooptekening kon niet geplaatst worden. Probeer een kleinere PNG/JPG.", marginX, y, maxW, 5);
      y += 4;
    }
  }
  ...
  const coreSentence = makeCoreSentence(sunSign, ascSign);

  const strength = `Kracht: ${SIGN_TEXT[sunSign]?.core || "jouw natuurlijke kernkwaliteit"} + ${SIGN_TEXT[ascSign]?.core || "jouw uitstraling"}`;
  const pitfall  = `Valkuil: ${SIGN_TEXT[moonSign]?.shadow || "een terugkerend emotioneel patroon"}`;

  const rawKey = SIGN_TEXT[moonSign]?.key || "Sleutel: kies een volwassen sleutel voor balans";
  const cleanKey = stripPrefix(rawKey, "Sleutel:");
  const key = `Sleutel: ${cleanKey}`;

  const lpTarot = getLifePathTarot(numerology.lifePath);
  const lpKey = normalizeLifePathForTarot(numerology.lifePath);

  const tarotBlock = lpTarot
    ? `\n\nLevenspad & archetype (Tarot)\nLevenspad ${numerology.lifePath}${numerology.lifePath === 33 ? " (basis 6)" : ""} — ${lpTarot.card}\n${lpTarot.text}`
    : "";

  y = addWrappedText(doc,
    `${coreSentence}\n\nZon in ${sunSign}: ${SIGN_TEXT[sunSign]?.core || ""}.\nMaan in ${moonSign}: ${SIGN_TEXT[moonSign]?.emotion || ""}\nASC in ${ascSign}: ${SIGN_TEXT[ascSign]?.core || ""}.\n\n${strength}\n${pitfall}\n${key}\n\nNumerologie: Life Path ${numerology.lifePath} — ${makeNumerologyThread(numerology.lifePath)}${tarotBlock}`,
    marginX, y, maxW, 5
  );
  ...
  if(totalPages >= 2){
    ...
    for(const pName of corePlanets){
      ...
      const keyClean = stripPrefix(st?.key || "", "Sleutel:");
      const line3 =
        (pName === "Maan")
          ? `Emotionele stijl: ${st?.emotion || ""}\nSchaduw: ${st?.shadow || ""}\nSleutel: ${keyClean}`
          : (pName === "Venus")
  ...
  if(totalPages >= 4){
    ...
    y = addWrappedText(doc,
      ...
      `Schaduw: ${SIGN_TEXT[venusSign]?.shadow}\n` +
      `Sleutel: ${stripPrefix(SIGN_TEXT[venusSign]?.key, "Sleutel:")}\n\n` +
    ...
    y = addWrappedText(doc,
      ...
      `Sleutel: ${stripPrefix(SIGN_TEXT[marsSign]?.key, "Sleutel:")}\n\n` +
      `Reflectievragen:\n• ${questionsForDrive()[0]}\n• ${questionsForDrive()[1]}`,
      marginX, y, maxW, 5
    );
  }
  ...
  if(totalPages >= 5){
    ...
    y = addWrappedText(doc,
      ...
      `Sleutel: ${stripPrefix(SIGN_TEXT[jSign]?.key, "Sleutel:")}`,
      marginX, y, maxW, 5
    );
    ...
    y = addWrappedText(doc,
      ...
      `Sleutel: ${stripPrefix(SIGN_TEXT[sSign]?.key, "Sleutel:")}`,
      marginX, y, maxW, 5
    );

    y += 4;
    y = addSectionTitle(doc, "Micro-acties (7 dagen)", marginX, y);

    const ascKeyRaw = SIGN_TEXT[ascSign]?.key || "Sleutel: werk met focus.";
    const ascKeyClean = stripPrefix(ascKeyRaw, "Sleutel:");

    const acts = [
      `Kies 1 focus (geen 3): maak het af. (${ascKeyClean})`,
      "Plan 1 eerlijk gesprek: benoem jouw behoefte in 1 zin.",
      "Maak 1 structuur-anker: vaste tijd/plek voor 15 minuten per dag."
    ];

    y = addWrappedText(doc,
      `• ${acts[0]}\n• ${acts[1]}\n• ${acts[2]}\n\nReflectievragen:\n• ${questionsForTalent()[0]}\n• ${questionsForTalent()[1]}`,
      marginX, y, maxW, 5
    );
  }
