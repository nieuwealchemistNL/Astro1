...
const SIGN_TEXT = {
  ...
  Stier: {
    ...
    emotion: "Emoties zijn diep, loyaal en gericht op innerlijke rust en zekerheid.",
    relation: "Zoekt betrouwbaarheid, nabijheid en continuïteit.",
    action: "Handelt langzaam maar vastberaden, gericht op behoud en opbouw.",
  ...
  Tweelingen: {
    ...
    emotion: "Emoties worden vaak mentaal verwerkt en via woorden geordend.",
  ...
  Kreeft: {
    ...
    emotion: "Emoties zijn intens en verbonden met veiligheid, herinnering en sfeer.",
  ...
  Leeuw: {
    ...
    action: "Handelt zelfverzekerd en met natuurlijke leiderschapsenergie.",
  ...
  Maagd: {
    ...
    action: "Handelt nauwkeurig en gericht op verbetering en ritme.",
    shadow: "Valkuilen zijn perfectionisme, zelfkritiek en overcontrole."
  },
  Weegschaal: {
    core: "Harmonie, schoonheid en relationele afstemming.",
    emotion: "Emoties worden gespiegeld via relaties en sociale dynamiek.",
    relation: "Zoekt gelijkwaardigheid, tact en balans.",
    action: "Handelt diplomatiek en afwegend, met oog voor het geheel.",
    shadow: "Valkuilen zijn besluiteloosheid en conflictvermijding ten koste van jezelf."
  },
  Schorpioen: {
    core: "Diepgang, transformatie en emotionele intensiteit.",
    emotion: "Emoties worden diep en vaak verborgen beleefd; niets is ‘licht’.",
    relation: "Zoekt absolute eerlijkheid, loyaliteit en emotionele binding.",
    action: "Handelt krachtig en doelgericht, met innerlijke vastberadenheid.",
  ...
  Boogschutter: {
    ...
    action: "Handelt enthousiast en visionair, gedreven door betekenis.",
    shadow: "Valkuilen zijn rusteloosheid en het vermijden van diepte of verantwoordelijkheid."
  },
  Steenbok: {
    core: "Structuur, verantwoordelijkheid en innerlijke autoriteit.",
    emotion: "Emoties worden beheerst en pragmatisch benaderd.",
    relation: "Zoekt betrouwbaarheid, respect en gedeelde doelen.",
    action: "Handelt gedisciplineerd, planmatig en resultaatgericht.",
    shadow: "Valkuilen zijn verharding, werkvlucht en emotionele afsluiting."
  },
  Waterman: {
    core: "Originaliteit, vrijheid en vernieuwend bewustzijn.",
    emotion: "Emoties worden mentaal benaderd en soms op afstand gehouden.",
    relation: "Zoekt gelijkwaardigheid, vriendschap en mentale verbinding.",
    action: "Handelt vernieuwend en onafhankelijk, volgens eigen waarden.",
  ...
  Vissen: {
    ...
    emotion: "Emoties worden diep meegevoeld en opgenomen uit de omgeving.",
    relation: "Zoekt zielsverbinding en energetische resonantie.",
    action: "Handelt intuïtief en meebewegend, vanuit innerlijke signalen.",
    shadow: "Valkuilen zijn escapisme, slachtofferschap en verlies van grenzen."
  }
};
...
function makeCoreSentence(sunSign, ascSign, lifePath) {
  return `De kern van deze persoon wordt gevormd door de levenskracht van ${sunSign}, die zich naar buiten toont via ${ascSign}. ${makeNumerologyThread(lifePath)}`;
}
...
function numerologyFromDate(y, m, d) {
  ...
  return { lifePath: lp, day: reduceToDigit(d), month: reduceToDigit(m), year: reduceToDigit(y) };
}
...
function renderPlanets(planets) {
  ...
  for (const p of planets) {
    ...
    tr.innerHTML = `<td>${p.label}</td><td>${fmtDeg(p.lon)}</td><td>${s.sign} ${fmtDeg(s.within)}</td>`;
...
function renderHouses(cusps, ascLon, mcLon) {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    const s = toSign(lon);
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${i}</td><td>${fmtDeg(lon)}</td><td>${s.sign} ${fmtDeg(s.within)}</td>`;
...
async function calculate() {
  ...
  try {
    ...
    let cusps, ascmc;
    if (Array.isArray(houseRes)) [cusps, ascmc] = houseRes;
    else {
...
function generatePdf() {
  ...
  if (totalPages >= 4) {
    ...
    y = addWrappedText(
      doc,
      `Emotionele stijl (${moonSign})\n${SIGN_TEXT[moonSign]?.emotion}\nValkuil: ${SIGN_TEXT[moonSign]?.shadow}\n`,
      marginX, y, 180, 5
    );

    y = addWrappedText(
      doc,
      `Relatiebehoefte (${venusSign})\n${SIGN_TEXT[venusSign]?.relation}\nValkuil: ${SIGN_TEXT[venusSign]?.shadow}\n`,
      marginX, y, 180, 5
    );

    y = addWrappedText(
      doc,
      `Actie & wil (${marsSign})\n${SIGN_TEXT[marsSign]?.action}\nValkuil: ${SIGN_TEXT[marsSign]?.shadow}`,
      marginX, y, 180, 5
    );
  }
  ...
  if (totalPages >= 5) {
    ...
    const lesson = SATURN_TEXT[saturnSign] ||
      "De kernles ligt in het ontwikkelen van innerlijke autoriteit, gezonde grenzen en duurzame structuur.";
